  ...
  update: function(dt) {
    ...
    if (this.stepAccumulator >= this.stepDelay) {
      ...
      if (this.position.y > CANVAS_WIDTH - 50) {
        ...
        document.querySelector("span#data-game").dataset.endgame = 'reset';
  ...
  for (var i = 0, len = aliens.length; i < len; i++) {
    ...
    if (alien.bullet !== null && checkRectCollision(alien.bullet.bounds, player.bounds)) {
      if (player.lives === 0) {
       ...
       particleManager.createExplosion(player.position.x, player.position.y, 'transparent', 100, 8,8,6,0.001,40);
